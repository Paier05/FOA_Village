...
const EventsTimerAdjustControl = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            setLoading(true);
            await axiosInstance.put("/apr/eventstime", { time: Number(minutes) });
            alert(`突发事件倒计时已设定为 ${minutes} 分钟`);
            setMinutes("");
        } catch (err) {
    ...
    return (
        <div className="admin-phase-controls">
            <h3 className="phase-header">设定突发事件新时长</h3>
...
export default EventsTimerAdjustControl;
